...
int nufs_getattr(const char *path, struct stat *st) {
  int rv = 0;
    rv = storage_stat(path, st); //get object attribute  
...
int nufs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  int len = strlen(path);
  char tmppath[len + 100];
  
  slist_t *dirlst = storage_list(path);
  //Loop through the directory list
   while(dirlst != NULL) {
        strcpy(tmppath, path);
        strcat(tmppath, dirlst->data);
        
        nufs_getattr(tmppath, &st);
        filler(buf, dirlst->data, &st, 0);
